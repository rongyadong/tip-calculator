...
export const selectTip: StoreSelector<number> = state => state.bill * (state.percentage / 100);

export const selectPerPerson: StoreSelector<number> = state => (state.bill + state.bill * (state.percentage / 100)) / state.split;